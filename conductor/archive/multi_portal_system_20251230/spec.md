...
This track transitions the application from a single, hardcoded portal system to
a dynamic, multi-portal architecture. It introduces a "World Registry" that acts
as a mock server, storing portals associated with specific worlds. Users will be
able to create portals dynamically, and the system will support navigating
between different worlds, each containing its own set of portals.

## Functional Requirements

1. **World Registry (Mock Server):**
   - Implement a central data structure (Registry) that maps a `WorldID`
     (or URL) to a list of `Portal` objects.
   - The initial starting area (the Lobby) will be treated as the "Hub World"
...
2. **Dynamic Portal Definition:**
   - A Portal object must contain:
     ...
     - `url`: The Marble API URL for the destination world (can be `null` for
       "empty" portals).
   - Other properties (name, assets) should be derived or fetched upon interaction.
3. **Portal Creation Workflow (Spawn Empty -> Edit):**
   - Pressing the 'P' key spawns a new `Portal` entry in the current world with
     `url: null` at the player's current position.
...
4. **Multi-Portal Rendering:**
   - The scene must render all portals associated with the current `WorldID` from
     the Registry.
...
- **Referential Stability:** Ensure that transitioning between worlds doesn't
  cause WebGL context loss.
- **State Management:** Utilize Zustand to manage the Registry (new slice) and
...
- Entering a portal correctly swaps the scene and loads the next set of portals
  from the Registry.
